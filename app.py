...
def make_pdf_report(inputs_pretty, results):
    pdf = FPDF()
    pdf.add_page()
    ...
    for key, value in inputs_pretty.items():
        pdf.cell(0, 10, f"{key}: {value}", ln=True)

    pdf.ln(5)
    pdf.cell(0, 10, f"Result: {results}", ln=True)

    # ------------------------- FIX HERE ------------------------------
    # Create a real temp file on Windows/Linux/Mac
